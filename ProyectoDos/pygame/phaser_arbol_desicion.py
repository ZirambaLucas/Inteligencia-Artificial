...
# Función para entrenar los modelos de árboles de decisión
def entrenar_modelo():
    global modelo_nn, modelo_nn_delantero, modelo_nn_izquierdo

    if len(datos_modelo) < 30:
        print("Hacen falta datos de entrenamiento.")
        return

    print("Entrenando modelo de", len(datos_modelo), "datos...")
    ...
    # Entradas y salidas para movimiento izquierdo
    X_izquierdo = np.array([[v2, d2] for _, _, _, v2, d2, _, i in datos_modelo])
    Y_izquierdo = np.array([i for _, _, _, _, _, _, i in datos_modelo])
    ...
    # Modelo de árbol para movimiento izquierdo
    modelo_nn_izquierdo = DecisionTreeClassifier()
