...
bala_img = pygame.transform.scale(pygame.image.load('assets/sprites/purple_ball.png'), (17, 17))
...
def manejar_salto():
    ...
    if salto:
        ...
        # Si el jugador llega al suelo, resetear todo
        if jugador.y >= h - 100:
            jugador.y = h - 100
            salto = False
            salto_altura = 15
...
def guardar_datos():
    ...
    izquierdo_hecho = 1 if izquierdo else 0  # 1 si se movió a la izquierda, 0 si no se movió
    
    # Guardar velocidad de la bala, distancia al jugador y si saltó o no
    datos_modelo.append((velocidad_bala, distancia, salto_hecho, velocidad_bala2, distancia2, delantero_hecho, izquierdo_hecho))
...
# Datos para salto (usamos velocidad_bala y distancia_bala_jugador)
    X_salto = np.array([[d[0], d[1]] for d in datos_modelo])  # velocidad_bala, distancia_bala_jugador
    Y_salto = np.array([d[2] for d in datos_modelo])  # salto

    # Datos para movimiento delantero (usamos velocidad_bala2 y distancia_bala2_jugador)
    X_delantero = np.array([[d[3], d[4]] for d in datos_modelo])  # velocidad_bala2, distancia_bala2_jugador
    Y_delantero = np.array([d[5] for d in datos_modelo])  # movimiento_delantero

    # Datos para movimiento izquierdo (usamos velocidad_bala2 y distancia_bala2_jugador)
    X_izquierdo = np.array([[d[3], d[4]] for d in datos_modelo])  # velocidad_bala2, distancia_bala2_jugador
    Y_izquierdo = np.array([d[6] for d in datos_modelo])  # movimiento_izquierdo
    ...
    modelo_nn_izquierdo = KNeighborsClassifier(n_neighbors=3)
